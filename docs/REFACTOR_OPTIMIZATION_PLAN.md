...
### 2026-05-02 A98 Frontend 租户版本权限页面

本阶段继续补齐动态菜单已引用但 `frontend/src/views` 缺失的系统页面，重建租户版本权限绑定入口。后端已有 `TenantEditionPermissionQueryService` / `TenantEditionPermissionAppService`，前端已有租户版本权限 API 合同，因此本阶段只新增页面和更新本文档，不修改 packages、不修改后端、不修改 Framework，也不暂存并行改动中的 tenant API 文件。

执行结果：

- 新增 `frontend/src/views/system/tenant-edition-permission/index.vue`：
  - 使用现有 `tenantEditionPermissionApi` 对齐租户版本权限查询、授权、状态更新和撤销动态 API。
  - 使用 `tenantEditionApi.enabledList()` 加载启用租户版本，作为权限绑定的版本上下文。
  - 使用 `permissionApi.availableGlobal()` 远程选择已启用全局权限，支持按关键字、模块编码和权限类型收窄候选项。
  - 使用 VxeGrid 展示当前版本权限绑定，支持关键字、模块编码、权限类型、授权状态和仅有效绑定筛选。
  - 提供授权权限、启用/停用绑定、撤销绑定入口；启用绑定时对已禁用权限做前端提示，最终仍以后端服务校验为准。

设计约束：

- 租户版本权限页维护平台版本白名单，不接收或传递当前租户上下文标识。
- 绑定查询只按租户版本主键读取后端已授权权限；关键字、模块和类型筛选在前端对列表展示做收窄，不引入新的后端查询契约。
- 授权入口只使用全局权限选择器，符合后端仅允许平台全局权限绑定到租户版本的规则。
- 撤销沿用后端当前硬删命令，不在前端新增兼容状态或软撤销字段。
...
- `pnpm lint`：通过，仍保留 packages 既有 24 个 `ts/no-explicit-any` 警告；本阶段新增 `src` 文件无 lint error。
- `pnpm build`：通过；构建仅保留 Tailwind content pattern、SignalR PURE 注释和大 chunk 既有警告。
- `Invoke-WebRequest http://127.0.0.1:7777/src/views/system/tenant-edition-permission/index.vue`：HTTP 200。
- `Invoke-WebRequest http://127.0.0.1:7777/system/tenant-edition-permission`：HTTP 200。
- `rg -n "\bany\b" frontend/src -g "*.ts" -g "*.vue"`：0 个匹配。
- `rg -n "\btenantId\b|TenantId" frontend/src -g "*.ts" -g "*.vue"`：仍只有既有 `TenantSwitcherDto.tenantId` 响应字段匹配；本阶段未新增租户请求字段。

协作状态：

- 阶段前检查 `XiHan.BasicApp` 已提交至 A97，工作区存在多项并行前端改动，不属于本阶段，未暂存未提交。
- `XiHan.Framework` 在提交前状态仍存在未跟踪 `framework/src/analysis.md`，不是本阶段改动，未暂存未提交。
- `XiHan.Framework` 本阶段无我方代码改动。
...
### 2026-05-02 A104 Frontend 用户直授权限 API 与页面

本阶段继续补齐后端已重建但前端缺失的授权绑定入口，重建 `UserPermission` 前端 API 和 `system/user-permission` 页面。后端已有 `UserPermissionQueryService` / `UserPermissionAppService`，因此本阶段只在 `frontend/src` 增加 API 合同与页面，不修改 packages、不修改后端、不修改 Framework，也不暂存并行改动中的其他 authorization API 文件。

执行结果：

- 扩展 `frontend/src/api/modules/authorization/types.ts`：
  - 新增 `UserPermissionListItemDto`、`UserPermissionDetailDto`、`UserPermissionGrantDto`、`UserPermissionUpdateDto`、`UserPermissionStatusUpdateDto`。
  - 复用 `PermissionAction`、`PermissionType`、`EnableStatus`、`ValidityStatus` 既有枚举，并通过类型引用复用租户成员类型/邀请状态。
- 新增 `frontend/src/api/modules/authorization/user-permission.ts`：
  - `userPermissionApi.list()` 调用 `UserPermissionQuery/UserPermissions/{userId}`，支持 `onlyValid` 查询参数。
  - `userPermissionApi.detail()` 调用 `UserPermissionQuery/UserPermissionDetail/{id}`。
  - `userPermissionApi.grant()` / `update()` / `updateStatus()` 对齐 `UserPermissionAppService`。
  - `userPermissionApi.revoke()` 调用 `UserPermission/UserPermission/{id}`，沿用后端撤销即写入无效状态的语义。
- 更新 `frontend/src/api/modules/authorization/index.ts` 导出用户直授权限模块。
- 新增 `frontend/src/views/system/user-permission/index.vue`：
  - 使用 `tenantMemberApi.page()` 选择当前租户内已接受、有效且非平台管理员的成员。
  - 使用 `permissionApi.page()` 选择已启用权限，支持按关键字、模块编码和权限类型收窄候选项；不强制只选全局权限。
  - 使用 VxeGrid 展示指定用户的直授权限绑定，支持关键字、模块编码、权限类型、授权动作、绑定状态和仅有效绑定筛选。
  - 提供直授权限、编辑授权动作/有效期/原因/备注、启用/停用绑定、撤销绑定入口。

设计约束：

- 用户直授权限 API 和页面不接收或传递当前租户上下文标识；当前租户由会话上下文、后端授权和仓储过滤器控制。
- `userId` 是当前租户成员身份下的用户资源主键，不是租户上下文；页面先通过租户成员查询入口选择用户。
- 平台管理员成员在前端做入口过滤与提示，最终仍以后端 `UserPermissionAppService` 校验为准。
- 后端用户直授权限列表接口按用户主键返回集合，关键字、模块、类型、动作和状态筛选在前端对列表展示做收窄，不引入新的后端查询契约。
...
- `pnpm lint`：通过，仍保留 packages 既有 24 个 `ts/no-explicit-any` 警告；本阶段新增 `src` 文件无 lint error。
- `pnpm build`：通过；构建仅保留 Tailwind content pattern、SignalR PURE 注释和大 chunk 既有警告。
- `Invoke-WebRequest http://127.0.0.1:7777/src/views/system/user-permission/index.vue`：HTTP 200。
- `Invoke-WebRequest http://127.0.0.1:7777/system/user-permission`：HTTP 200。
- `rg -n "\bany\b" frontend/src -g "*.ts" -g "*.vue"`：0 个匹配。
- `rg -n "\btenantId\b|TenantId" frontend/src -g "*.ts" -g "*.vue"`：仍只有既有 `TenantSwitcherDto.tenantId` 响应字段匹配；本阶段未新增租户请求字段。
- `git diff --check`：通过，仅提示既有工作区换行符规范警告。
...
- 阶段前检查 `XiHan.BasicApp` 已提交至 A103，工作区存在多项并行前端改动，不属于本阶段，未暂存未提交。
- `XiHan.Framework` 在提交前状态仍存在未跟踪 `framework/src/analysis.md`，不是本阶段改动，未暂存未提交。
- `XiHan.Framework` 本阶段无我方代码改动。
...
### 2026-05-02 A106 Frontend 角色数据范围 API 与页面

本阶段继续补齐后端已重建但前端缺失的授权绑定入口，重建 `RoleDataScope` 前端 API 和 `system/role-data-scope` 页面。A105 已补齐 `organization/department` 只读树 API，本阶段直接复用部门选择器；范围限定在 `frontend/src` 和本文档，不修改 packages、不修改后端、不修改 Framework，也不暂存并行改动中的其他文件。

执行结果：

- 扩展 `frontend/src/api/modules/authorization/types.ts`：
  - 新增 `RoleDataScopeListItemDto`、`RoleDataScopeDetailDto`、`RoleDataScopeGrantDto`、`RoleDataScopeUpdateDto`、`RoleDataScopeStatusUpdateDto`。
  - 复用 `DepartmentType`、`EnableStatus`、`ValidityStatus` 既有枚举，并保留角色数据范围的有效期字段。
- 新增 `frontend/src/api/modules/authorization/role-data-scope.ts`：
  - `roleDataScopeApi.list()` 调用 `RoleDataScopeQuery/RoleDataScopes/{roleId}`，支持 `onlyValid` 查询参数。
  - `roleDataScopeApi.detail()` 调用 `RoleDataScopeQuery/RoleDataScopeDetail/{id}`。
  - `roleDataScopeApi.grant()` / `update()` / `updateStatus()` 对齐 `RoleDataScopeAppService`。
  - `roleDataScopeApi.revoke()` 调用 `RoleDataScope/RoleDataScope/{id}`，沿用后端撤销即写入无效状态的语义。
- 更新 `frontend/src/api/modules/authorization/index.ts` 导出角色数据范围模块。
- 新增 `frontend/src/views/system/role-data-scope/index.vue`：
  - 使用 `roleApi.page()` 按 `DataPermissionScope.Custom` 和启用状态选择可维护角色，并过滤平台全局角色与系统角色。
  - 使用 `departmentApi.tree()` 选择启用部门。
  - 使用 VxeGrid 展示指定角色的数据范围部门绑定，支持关键字、绑定状态和仅有效绑定筛选。
  - 提供授予范围、编辑包含子部门/有效期/备注、启用/停用绑定、撤销绑定入口。

设计约束：

- 角色数据范围 API 和页面不接收或传递当前租户上下文标识；当前租户由会话上下文、后端授权和仓储过滤器控制。
- `roleId` 是角色资源主键，不是租户上下文；页面只允许选择当前上下文可见且数据范围为自定义的启用角色。
- 部门树仅作为选择器读模型，不在前端实现组织权限裁决；停用部门恢复有效绑定时前端只做提示，最终以后端校验为准。
- 角色数据范围绑定不提交 `DataPermissionScope`，角色是否为自定义范围由后端角色实体最终校验。
...
- `pnpm lint`：通过，仍保留 packages 既有 24 个 `ts/no-explicit-any` 警告；本阶段新增 `src` 文件无 lint error。
- `pnpm build`：通过；构建仅保留 Tailwind content pattern、SignalR PURE 注释和大 chunk 既有警告。
- `Invoke-WebRequest http://127.0.0.1:7777/src/views/system/role-data-scope/index.vue`：HTTP 200。
- `Invoke-WebRequest http://127.0.0.1:7777/system/role-data-scope`：HTTP 200。
- `rg -n "\bany\b" frontend/src -g "*.ts" -g "*.vue"`：0 个匹配。
- `rg -n "\btenantId\b|TenantId" frontend/src -g "*.ts" -g "*.vue"`：仍只有既有 `TenantSwitcherDto.tenantId` 响应字段匹配；本阶段未新增租户请求字段。
- `git diff --check`：通过，仅提示既有工作区换行符规范警告。
...
- 阶段前检查 `XiHan.BasicApp` 已提交至 A105，工作区存在多项并行前端改动，不属于本阶段，未暂存未提交。
- `XiHan.Framework` 在提交前状态仍存在未跟踪 `framework/src/analysis.md`，不是本阶段改动，未暂存未提交。
- `XiHan.Framework` 本阶段无我方代码改动。
...
本阶段处理系统页面中 VxeTable 只显示工具栏、表头和分页，表格主体区域没有铺满剩余高度的问题。回溯重构前提交 `e052db5f` 后确认旧版 `frontend/packages/app.vue` 的 `NConfigProvider` 带有 `class="h-full"`，重建到 `frontend/src/App.vue` 后该高度入口丢失，导致 BasicLayout、页面根和 `vxe-card.flex-1` 都按内容高度计算，VxeGrid 只能得到很小的父级高度。

执行结果：

- 更新 `frontend/src/App.vue`：
  - 恢复重构前 `NConfigProvider class="h-full"`，重新建立从 `#app` 到 BasicLayout 的完整高度链。
- 更新 `frontend/packages/layouts/basic/index.vue`：
  - 回退上一版错误的 `h-full min-h-0` 调整，恢复重构前 `min-h-full`、`flex-1 overflow-auto`、内容层 `h-full` 写法。
  - 避免外层 provider 高度未稳定时把 BasicLayout 强行压缩到内容高度。
- 更新 `frontend/packages/design/global.css`：
  - `vxe-card` 补齐 `min-width: 0`，避免表格列和工具栏横向溢出影响布局。
  - 针对 `vxe-card.flex-1` 表格容器覆盖页面历史内联 `height: 0` 对百分比高度计算的影响，并补齐 body/content/grid 的 flex 与高度传递。
...
- `pnpm type-check`：通过。
- `pnpm build:dev`：通过。
- Playwright + Edge 本地验证 `#/system/permission`：`NConfigProvider`、BasicLayout、内容区恢复到 630px 视口高度，`vxe-card.flex-1` 从 162px 提升到 401px，表格主体恢复显示。
- 构建仍保留 SignalR 依赖 PURE 注释警告，属于第三方包注释位置问题，本阶段不处理。
