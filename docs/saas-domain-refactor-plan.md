...
| Sub-Batch | 标题 | 范围 | 完成标准 |
|-----------|------|------|----------|
| Batch-09A | 角色与部门操作租户语义收敛 | `RoleAppService`、`DepartmentAppService`、相关 Query/Repository | 角色权限/继承/数据范围与部门创建入口统一按显式租户或当前会话租户执行，不再把实体主租户字段混作操作租户 |
| Batch-09B | 菜单树与权限绑定收敛 | `MenuAppService`、`MenuQueryService`、菜单 DTO/前端 API | 菜单纯 UI 结构与权限绑定关系分离清晰，菜单树构建不再混入旧权限假设 |
| Batch-09C | 数据范围与部门闭包语义收敛 | `PermissionDomainService`、`OrganizationDomainService`、`Department/Role` 相关查询与前端入口 | 角色数据范围、部门闭包、继承展开前后端语义一致，避免范围放大或树坏链 |
...
- [x] Batch-08B 注册与外部登录成员链补齐
- [x] Batch-08C 用户与租户管理用例收口

### Batch-09 Sub-Status

- [x] Batch-09A 角色与部门操作租户语义收敛
- [x] Batch-09B 菜单树与权限绑定收敛
- [ ] Batch-09C 数据范围与部门闭包语义收敛
...
| Batch-09 | `dotnet build` / 安全出口自检 | Partial | Batch-09A/09B 已完成前两段收口：09A 将 `RoleAppService` 的角色权限分配、继承分配、数据范围分配与角色创建，以及 `DepartmentAppService` 的部门创建入口统一切到显式租户或当前会话租户语义；09B 则修复了 `MenuAppService` 创建/更新未回写 `PermissionId/IsGlobal/Metadata` 的问题，并将 `MenuQueryService` 的全量菜单、角色菜单、用户菜单统一改为树形 UI 结构输出，避免继续把平铺菜单列表交给前端二次拼树。`dotnet build` 仍受本机 .NET SDK workload 异常阻塞，只输出“生成失败，0 个警告 0 个错误”，当前结论仍以代码事实扫描为主 |
...
| Batch-07C | 初始化链自检 / `dotnet build` 定向验证 | Partial | 已将 `SysTenant/SysDepartment/SysDepartmentHierarchy/SysUserRole/SysDict/SysDictItem` 改为按业务键补齐的默认租户初始化链：默认租户不再因库中存在其它租户而整体跳过，默认部门树改为围绕 `ROOT/OPS/DELIVERY/SECURITY` 构建，部门闭包表改为每次全量重建，`SysUserRoleSeeder` 已区分平台角色绑定与默认租户角色绑定，内置字典新增租户成员类型、邀请状态、数据权限范围等模板。针对性扫描仅保留新模板名与新角色码，不再命中旧 `TECH/PRODUCT/OPERATION/ADMIN` 结构或 `employee/guest` 角色依赖。`dotnet build` 仍受本机 SDK workload 环境问题阻塞，仅输出“生成失败，0 个警告 0 个错误” |
