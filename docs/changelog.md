...
- **连接与缓存的默认节点名统一为 `Default`**（跟随框架 v4.1.0）：数据库 `ConfigId` 由 `"1"` 改为 `"Default"`，升级模块的 `ConnectionConfigId` 同步；Redis 键前缀 `InstanceName` 由 `"XiHan:"` 改为 `"Default:"`；主从示例的第二连接由 `"2"` 改为 `"Report"`——纯数字 `ConfigId` 会被租户连接解析优先按 `tenantId.ToString()` 命中，租户被静默路由到该库。改过连接名或 Redis 键前缀的部署需同步改配置。
- **升级脚本挪进版本子目录**：框架的 `FileSystemUpgradeScriptProvider` 扫的是 `UpdateScripts/<版本>/*.sql`，而脚本一直平铺在根目录，四个脚本从写下起一条都没执行过、也没有任何报错。目录修正后，叠加框架 v4.1.0 补上的迁移执行，它们会在启动时按序全部执行；本版另随附 `4.0.1`（租户存储用量 SUM 改走覆盖索引）、`4.0.2`（四张表唯一索引补租户维度、AI 密钥列扩容）、`4.0.3`（`sys_user_session` 补模仿登录六列与索引）。**升级前请先备份数据库。**
- **页面按钮门控改由服务端下发按钮码**：`PermissionInfoDto` 新增 `Buttons` 字段，前端 141 处硬编码权限码全部退役。该字段缺省视为一个按钮都没有，前后端必须配套发布。
- **通知公告页的权限码由 `saas:notification:*` 改指 `saas:message:*`**：该页所有端点本就要求 `saas:message:*`，此前菜单按 `notification:read` 可见、进去每个请求 403。菜单种子在下次启动时更新既有行，无需迁移脚本。
- **旧前端 `old-frontend/` 已移除**；`frontend/` 的 `@xihan-ui` 由链源码切回 npm 正式版，catalog 钉 `^1.1.0`。锁文件不入库，catalog 是唯一的版本约束。
- **示例模块的 `Sample:TenantModuleDataSources` 配置节移除**：租户模块库交给框架约定，库隔离租户开箱就有自己的模块库；要指到别处，实现 `ISqlSugarTenantConnectionProvider` 显式给出 `ModuleDataSourceConfigs`（显式优先于约定）。
- **三个接口的标量主键改走查询串**：租户初始化数据库、第三方应用重置密钥、版本权限撤销。动态 API 对未标绑定特性的简单类型参数一律推断为 `[FromQuery]`，此前把 `{ id }` 放进请求体，后端收到的一直是 `0` 且连 400 都不报。
:::

- **新增** 模仿登录：超管与租户管理员可以他人身份登录排查问题，以被模仿者的完整身份重新签发令牌并新建一条带模仿者标记的独立会话行，鉴权、租户过滤、菜单裁剪与审计字段全部自动跟随。准入 fail-closed——禁止模仿超管、模仿自己、嵌套模仿、目标被禁用或非目标租户有效成员，跨租户另需 `saas:impersonation:cross-tenant`；模仿会话独立短过期（默认 30 分钟）、禁止刷新，退出时按令牌声明与会话行双来源交叉验证；模仿态禁用清单覆盖改密、重置 MFA、授权变更、删除、密钥再生成与再次模仿。开始与结束各落一条登录日志，操作日志描述拼上模仿者，按配置向被模仿者投递安全通知
- **新增** 模仿登录前端入口：用户菜单「控制中心」之后发起，模仿期间顶部横幅退出，用户列表提供行操作
- **新增** 示例业务模块 `backend/src/business/XiHan.BasicApp.Sample`：clone 下来第一个可读的最小业务模块（一个模块类 + 两个实体 + 两个仓储，表由框架启动时按实体自动建），并演示「模块分库 × 租户分库」二维路由与「加一块自己的业务要写哪些东西」
- **新增** 租户席位与存储配额的运行时拦截：`UserLimit` / `StorageLimit` 此前只校验入参、没有任何执行点，配额是纯装饰。席位拦在 `UserDomainService.CreateUserAsync`（三个应用层入口全汇聚于此），存储拦在 `FileDomainService.CreateUploadingFileAsync`（传输开始前就拒绝）；另补存量配额核对接口与租户页「配额核对」入口
- **调整** 拖拽排序换成 `@xihan-ui/vue` 的 sortable，六处排序列表全部切过来，`@dnd-kit/vue` 与 `@dnd-kit/geometry` 连同两个包装件一并退役
- **调整** 表格列宽改用组件库的 `XhTableColumnResizeTrigger`，手写那套整份移除；顺带修掉「第一次拖动瞬间整排列宽跳一下」——原先钉住的是声明宽度而非屏幕上的实际宽度
- **修复** 封堵模仿态逃逸口与授权侧门（对抗审查确认 21 条）：切换租户、OAuth 同意流、令牌端点的 `refresh_token` 三条「用既有会话重签令牌」的路径不透传 `impersonator_*` 声明，等于把整套约束洗掉；刷新链的模仿拒绝与会话校验挂在按有效期校验的解析上，在唯一会触发刷新的场景里一起空转；停用、删除、强制下线发起人吊销不掉其模仿会话（会话行的 `UserId` 是被模仿者）；权限申请审批与权限委托直授权限 / 角色却不经授权准入
- **修复** 改密与锁定后吊销会话：安全戳 `SysUserSecurity.SecurityStamp` 只被写、全仓没有读取方，管理员重置密码、锁定账号与自助改密之后旧令牌一直能用到会话自然过期，等于密码白改、锁定不生效
- **修复** 租户开通改在平台态执行：整段开通被包在新租户上下文里，而库隔离租户创建出来时独立库还没建，开通期第一次查库就打到那个库上（3D000 / 42P01）；`ExistsEmailGloballyAsync` 契约是全平台判重却跟着当前上下文走，全局唯一形同虚设。开通与套餐回收全程走平台态，账号注册表与授权绑定落平台库
- **修复** 租户开通时把模块库一起建出来（改调框架的 `InitializeCurrentLayoutAsync`），此前只作用于租户主库
- **修复** 租户停用与成员撤销后缓存未失效：租户被暂停 / 过期 / 禁用后只改了库没失效会话状态缓存，被停的人最长 60 秒仍能通行；撤销租户成员身份后授权快照缓存未失效，被移除的角色权限继续生效到过期
- **修复** 四张表的唯一索引补上租户维度（工作流定义、代码生成的数据源 / 表 / 模板）：约束是全局的而读写两侧按租户过滤，租户 B 会撞上自己根本看不见的行，表现为「编码明明没被占用却建不出来」；三条代码生成索引一并补 `is_deleted`；AI `api_key` 列按密文长度扩容
- **修复** 前端一批安全与正确性缺陷：静态模式的路由级权限过滤整体失效、无权限用户拿得到全部路由；密文被当业务消息展示给用户、并发 401 重复登出；权限数组混入空串导致整组放行；草稿预览值被上行并推送到其它设备、`$reset` 静默退化；树工具丢数据与爆栈、`getParentIds` 在 `parentId` 成环时死循环卡死主线程；动态路由装载完成后重进丢参、装载持续失败时 `/500` 自我循环
- **修复** 聊天、AI、代码生成、打印与工作流的一批领域层缺陷：普通成员无法主动退群、保留期清理漏掉表情回应、NaN 温度被放行落库、解密失败的异常语义误导排查、枚举值不校验与备注被清空、校验缺口与异常参数名错位
- **修复** 导出中心「我的任务」等分页与枚举缓存的边界问题，`params` 位覆盖调用方配置，重复的重置密码入口
- **优化** 前端体验打磨：认证页与登录页的切换条加高、第三方渠道收进「更多」、提交钮与输入框同高且圆角跟随控件形状令牌；在线聊天输入框与表情选择框、表格样式优化；搜索条件的列宽跟着字号走；正文滚动区不再被页内最宽的表格撑破视口；四处藏着的交互补上就地提示；标签的悬停提示先说标签名
- **升级** 升级依赖，发布 v5.0.0
