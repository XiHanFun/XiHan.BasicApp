...
::: warning 破坏性变更：本版必须重建数据库
在线聊天与打印模板拆为独立一等模块，权限码整体更名（`saas:chat:*` → `chat:*`、`saas:print-template:*` → `print-template:*`），聊天配置键更名（`saas:chat:*` → `chat:*`）并新增种子，菜单目录调整（聊天迁出消息中心自持「聊天」目录、审批迁出工作流目录自持「审批」目录），种子新增 400/500/900 三个 Order 段。既有库中的角色授权与配置将静默失效，部署时必须重建数据库，无迁移脚本。
:::

- **调整** 在线聊天拆为独立模块 `XiHan.BasicApp.Chat`：30 个源文件迁出 Saas，权限/菜单/种子/Hub 端点自持；聊天保留期清理独立为专属任务，配置键 `chat:retention-days` / `chat:sensitive-words` 补种子；AI 助手桥接留在 AI 模块（删聊天须连带处理）
- **调整** 打印模板拆为独立模块 `XiHan.BasicApp.Printing`：权限/菜单/缓存/种子自持，全局管理经平台专属码登记点归入统一排除口径
- **调整** 前端新增 `src/modules/` 目录约定：五个可选模块（ai/chat/codegen/printing/workflow）的视图、API、文案与启动钩子各自收进模块目录，删目录即卸载、零接线；配套 validate-modules 构建门禁
- **调整** 聊天壳层整体下沉 `@xihan/chat` 功能包（组件/store/类型/常量/集成件/语音录制），布局层改经壳层扩展注册点挂载顶栏入口、全局抽屉与实时集成——聊天前端达到删目录即整体卸载
- **调整** AI 助手从聊天壳层解耦为提供方扩展点：AI 模块注册后助手入口才出现，删 AI 后聊天照常
- **调整** 打印数据源目录后端化：后端注册表为单一事实源（业务模块 ConfigureServices 注册、启动期校验、模板写路径 fail-closed 拦未注册绑定），前端设计器惰性拉取目录
- **修复** 打印引擎此前每次创建模板/启用拖拽都整体重初始化（连接配置被重置），改为数据源注册表版本号驱动；模块启动钩子热更新重跑不再因重复注册报错
- **调整** 审批菜单目录独立，消除审批与工作流模块的目录双登记
- **修复** 租户版本白名单新增重算种子（Order 900）：模块权限首启即进企业版白名单，消除「第二次启动才生效」的缺口（AI/工作流的存量同款缺口一并修复）
- **修复** 前端 i18n 校验脚本此前运行即崩（聚合文件路径错误），修复并把建模面扩到业务层与模块层全量文案
- **新增** 聊天模块补齐 21 个领域测试（会话/成员/撤回/敏感词/权限清单）；打印模块测试独立成工程

## v3.11.1 (2026-08-12)

- **新增** 打印模板设计器，可视化排版、数据源绑定、示例数据预览与打印机偏好，模板按租户隔离存储
- **修复** Redis 未启用时延迟队列不注册，导出与发件箱服务在容器解析阶段失败、应用无法启动；随框架 3.11.1 修复，单实例部署下二者改由进程内队列承载
