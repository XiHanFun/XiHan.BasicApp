...
- **新增** 前端单元测试基建（vitest，复用 `vite.config.ts` 的别名与自动导入），并补齐 utils / request / router / stores / hooks / domain / api 七层用例；后端为六个缺测源项目建立测试工程，补齐 Saas 领域层·应用层·基础设施层与 AI、代码生成、工作流、聊天、打印、Web.Core、WebHost、Core 的测试
...
- **修复** 授权完整性守卫漏登记 Chat 与 Printing 两个模块，其全部端点一直不受任何授权约束而测试照常全绿；新增「凡含动态 API 应用服务却不在册即失败」的守卫，判据复用框架的类型工具，与运行期生成控制器的集合保持一致
...
- **优化** CI 工作流的 action 升到各自最新大版本，一律跑 node24，`pnpm/action-setup` v6 起正式支持 pnpm v11，与 `frontend/package.json` 声明的 `packageManager` 对齐
- **调整** README 改造为双语，新增 backend / frontend 工程说明
- **升级** 升级依赖，发布 v5.0.0
...
后端移除 15 个从未落地的空壳示例模块工程（APS / BI / CMS / CRM / EAM / ERP / FMS / HRM / MES / OA / PLM / QMS / SCM / TMS / WMS），解决方案条目一并清理。
:::
...
- **新增** 补齐权限域测试：权限裁决、数据范围决策、角色继承链、三源权限合并、字段级安全与脱敏器、权限委托生命周期、约束规则不变量与执法、授予路径 SSD
...
- **修复** 修复前端 i18n 校验脚本运行即崩，校验面扩到业务层与模块层
- **升级** 升级依赖，发布 v3.12.0
...
- **修复** 前端 CI 依赖安装此前恒失败，锁文件与 pnpm 版本两处断点修复
- **升级** 升级依赖，发布 v3.11.1
...
- **调整** 应用文档迁入本仓 `docs/` 独立成站，更新日志归口文档站，仓库根 `CHANGELOG.md` 移除
- **调整** 国内镜像由 AtomGit 迁到 GitCode
