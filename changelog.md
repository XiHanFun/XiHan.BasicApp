来源：https://basicapp.docs.xihanfun.com/changelog
...
- **修复** 查询接口被记进操作日志：分页、「我的」列表、时间线等以 POST 发起的查询被记成新增 / 审核 / 修改，导出中心每轮询一次就多一条「新增」。查询服务的接口不再记操作日志，已写入的历史记录随保留策略过期
- **修复** 登录日志、操作日志与个人中心登录记录中，模仿登录、登录、登出、查询显示为「-」：登录结果与操作类型改取后端枚举文案，七种语言齐备；操作类型改为按语义着色的标签
- **修复** 聊天输入框多行正文溢出到发送行、外层多出一层描边：改为多行文本字段，默认 5 行、最多 10 行；群聊工具条新增「@」按钮打开成员列表
- **修复** 顶栏聊天未读角标的数字在红底上看不见
- **调整** 各日志页的链路追踪 ID 列位置统一：开放接口与异常日志排在请求标识之前，权限变更日志排在操作人之后
- **调整** 删除前端的登录结果、操作类型文案键（`log.login.result_*`、`log.operation.type_*`、`profile.login_logs.result_*`），二次开发引用处改用 `useEnumOptions` 取后端枚举
- **升级** 发布 v5.3.1

## v5.3.0 (2026-09-25)

::: warning 升级须知
本次重构多租户边界，含破坏性变更，依赖升至 XiHan.Framework 4.6.0，前后端须配套发布。**升级前请备份数据库。**

- **数据库**：随附 `UpdateScripts/5.3.0`，启动时在建表之后、播种之前自动执行，可重复执行；新建的库直接登记为最新版本，不再补跑历史脚本
- **库隔离租户**：平台库改为目录库，租户独立库只放租户业务数据；5.2.0 及更早开通的库隔离租户需按[多租户：库隔离](backend/multi-tenancy.md#库隔离-平台库是目录库)人工迁移
- **接口**：用户角色、直授权限、部门、数据范围与角色继承的单条授予 / 撤销接口删除，只留批量接口；建租户不再带管理员，改由 `InitializeTenantAdmin` 开通；隔离模式创建后不可修改
- **参数**：同一功能的参数合并为一条 JSON（`saas.auth.login`、`saas.auth.password`、`saas.auth.impersonation`、`saas.bot.telegram`、`chat.policy`），脚本自动合并旧值；强制改密改为 `saas.auth.password` 的 `forceChange`，默认关闭
- **配置**：移除 `Saas:Seed:SuperAdminPassword`，超管初始密码固定为 `SuperAdmin@123`；`Saas:Seed:EnableDemoData` 缺省改为关闭，需在各环境配置中写明
- **种子**：不再创建默认租户 `default`；菜单、权限等依赖缺失时启动直接失败
:::

- **新增** 权限作用侧（平台 / 租户 / 两侧）：租户内生效的权限 = 授权 ∩ 作用侧 ∩ 套餐白名单，超级管理员只在平台成立
- **新增** 租户所有者改为随套餐生效的系统角色，平台可转移所有权；租户侧新增「我的订阅」页
- **新增** 角色成员、角色授权与用户直授改用穿梭框批量维护；角色成员上限与职责分离对所有授予路径生效
- **新增** 角色与成员共用的数据范围编辑器，成员可按租户单独设置数据范围
- **新增** 页头显示当前上下文（平台 / 租户名）；模仿登录可先选范围
- **新增** 租户列表支持行级启用 / 暂停 / 禁用
- **新增** 演示数据按开关写入：10 个演示租户覆盖各套餐、租户状态、成员类型与数据范围，演示账号密码均为 `Demo@123`
- **新增** 前端构建产出 `version.json`，发版后才打开的页面也能收到更新提示
- **调整** 平台就是 0 号租户：平台与各租户只看、只写各自的数据；切换租户会换新会话，只有平台账号能进平台
- **调整** 账号归属注册地租户，外部成员在所在租户只能维护角色、部门等成员关系
- **调整** 全局角色、参数与字典在租户内只读，租户可建同键参数覆盖全局参数
- **调整** 带密钥的配置按租户隔离，邮件、短信与存储在租户未自配时回退平台默认
- **调整** AI 提示词、助手与知识库归平台维护；助手对话改由聊天权限控制
- **调整** 管理页写操作一律按按钮权限显示
- **修复** 平台专属权限在业务租户中同样生效
- **修复** 租户可读取平台的带密钥配置
- **修复** 更换默认存储后，既有文件按新存储取凭证导致无法下载或删除
- **修复** 后台导出未按发起人身份执行，会话下线或模仿态下权限与在线请求不一致
- **修复** 已停用角色仍参与数据范围过滤；撤销过的角色重新授予时报错
- **修复** 顶栏菜单下拉被裁切、切页进度条被顶栏遮住、窄栅格下仪表盘与编辑弹窗溢出
- **优化** 页面切换动画改用 Web Animations API，切页卡顿明显减少
- **升级** 升级依赖至 XiHan.Framework 4.6.0 与 XiHan.UI 2.1.0，发布 v5.3.0
