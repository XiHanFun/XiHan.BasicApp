...
public static class MenuPermissionArtifactGenerator
{
    ...
    private static GeneratedArtifact BuildReadme(CodeGenerationContext context, IReadOnlyCollection<string> collidingCodes)
    {
        ...
        sb.AppendLine($"| `{context.ClassName}PageRegistry.snippet.txt` | 粘贴到 Saas `PageRegistry`（按钮码的唯一来源，必做） | 参考片段 |");
        sb.AppendLine();
        ...
        sb.AppendLine("3. **登记页面与按钮**：把 `" + context.ClassName + "PageRegistry.snippet.txt` 的 `PageDescriptor` 条目粘进 `PageRegistry.All`、");
        sb.AppendLine("   `ButtonDescriptor` 条目粘进 `PageRegistry.Buttons`。**这一步不可跳过**：生成页面的新增/编辑/删除按钮");
        sb.AppendLine("   用的是按钮码 `" + Shared.ModuleLower(context) + "." + kebab + ".{create|update|delete}`，只由 `PageRegistry.Buttons` 下发；");
        sb.AppendLine("   `" + context.ClassName + "MenuSeeder` 只种菜单行、不种按钮行，光走它按钮会一个都不显示，");
        sb.AppendLine("   前端 `view-permission-hygiene` 门禁也会把这些码逐条列为「未登记」。");
        sb.AppendLine("   走 `PageRegistry` 单一事实源时，菜单本身也由 `PageDescriptor` 承载，可不注册 MenuSeeder。");
        sb.AppendLine("4. **重建数据库**：菜单、按钮、权限、超管授权即到位，页面可访问。");
        sb.AppendLine();
