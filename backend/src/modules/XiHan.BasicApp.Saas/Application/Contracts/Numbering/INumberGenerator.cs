...
/// </para>
/// <para>
/// <b>事务语义</b>：发号<b>总是</b>在自己的事务里完成并当场提交，语义对齐数据库序列。
/// 在调用方事务内调用时，框架会为发号物化一条独立物理连接，因此：
/// </para>
/// <list type="number">
///   <item>
///     <b>编号发出即永久消耗</b>，不随调用方事务回滚而回收。已经外泄的编号（写日志、打印、下发外部系统）
///     不可能被再次发给另一个业务实体。
///   </item>
///   <item>
///     <b>规则行的锁只在发号期间持有</b>，不会被调用方事务的剩余时长拖住，
///     因此调用方在取号之后再调用外部接口不影响该规则的吞吐。
///   </item>
///   <item>
///     <b>不要在已经修改过同一条规则行的事务里发号。</b>发号走的是另一条连接，
///     会等待调用方自己尚未提交的行锁，而这个等待环有一半在应用线程上，数据库的死锁检测看不到。
///     业务代码正常不会直接改编号规则行，这条只针对管理端与发号混在同一事务的写法。
///   </item>
/// </list>
/// <para>
...
/// </para>
/// <para>
/// <b>部署形态无关</b>：无论租户是共享库（行隔离）还是独立库，也无论用的是租户私有规则还是平台全局规则，
/// 发号都走同一条路径、同样独立提交。不存在「跨库时行为不同」的分叉，
/// 也不依赖工作单元对多条连接的顺序提交（框架不提供两阶段提交）。
/// </para>
/// <para>
