...
/// </para>
/// <para>
/// <b>库隔离租户调用平台全局规则</b>：全局规则行位于平台库，租户业务数据位于租户库，两者不在同一条连接上。
/// 此时并入调用方事务只是幻觉——框架不提供两阶段提交，顺序提交中若平台侧失败，
/// 调用方会拿到一个既未记录也未推进流水的编号，下一次发号必然重复它。
/// 因此跨库时发号<b>自行开启并提交事务</b>：编号在返回前即永久生效，不随调用方事务回滚而回收，
/// 调用方事务失败只会留下空洞。上文第 1 条「与调用方事务同生共死」在这种跨库场景下不适用。
/// 共享库（行隔离）部署中平台库与租户库是同一条连接，仍按加入调用方事务处理。
/// </para>
/// <para>
