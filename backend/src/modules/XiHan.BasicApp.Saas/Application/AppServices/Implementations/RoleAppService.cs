        ...
        IRoleAppService
{
    ...
    public async Task AssignPermissionsAsync(AssignRolePermissionsCommand command)
    {
        ...
        await _roleManager.AssignPermissionsAsync(
            ...
            permissionIds,
            resolvedTenantId);
        await SyncRoleButtonMenusAsync(role.BasicId, permissionIds, resolvedTenantId);
    ...
    private async Task SyncRoleButtonMenusAsync(long roleId, IReadOnlyCollection<long> permissionIds, long? tenantId)
    {
        ...
        var pageMenuIdSet = allMenus
            .Where(menu => menu.MenuType == MenuType.Menu && menu.Status == YesOrNo.Yes)
            .Select(menu => menu.BasicId)
            .ToHashSet();
        if (pageMenuIdSet.Count == 0)
        {
            return;
        }

        var buttonMenus = allMenus
            .Where(menu =>
            ...
            .ToList();
        var buttonMenuCodeMap = buttonMenus
            .GroupBy(menu => BuildScopedMenuCode(menu.TenantId, menu.MenuCode!), StringComparer.OrdinalIgnoreCase)
            .ToDictionary(
                group => group.Key,
                group => group
        ...
        var targetButtonMenuIds = new HashSet<long>();
        var targetParentMenuIds = new HashSet<long>();
        if (permissionIds.Count > 0 && buttonMenuCodeMap.Count > 0)
        {
            var permissions = await _permissionRepository.GetByIdsAsync(permissionIds);
            foreach (var permission in permissions)
            {
                if (!TryGetButtonPermissionCode(permission.PermissionCode, out var buttonPermissionCode))
                {
                    continue;
                }
                ...
                if (buttonMenuCodeMap.TryGetValue(
                    ...
                    || (permission.TenantId != 0
                        && buttonMenuCodeMap.TryGetValue(
                            ...
                            out buttonMenu)))
                {
                    targetButtonMenuIds.Add(buttonMenu.BasicId);
                    if (buttonMenu.ParentId.HasValue && pageMenuIdSet.Contains(buttonMenu.ParentId.Value))
                    {
                        targetParentMenuIds.Add(buttonMenu.ParentId.Value);
                    }
                }
            }
        }

        var existingRoleMenus = await _roleRepository.GetRoleMenusAsync(roleId, tenantId);
        var existingActiveMenuIds = existingRoleMenus
            .Where(mapping => mapping.Status == YesOrNo.Yes)
            .Select(mapping => mapping.MenuId)
            .Distinct()
            .ToArray();

        var autoManagedButtonMenuIds = allMenus
            .Where(menu =>
                menu.MenuType == MenuType.Button
                && menu.ParentId.HasValue
                && pageMenuIdSet.Contains(menu.ParentId.Value)
                && string.Equals(menu.Remark, AutoGeneratedButtonRemark, StringComparison.Ordinal))
            .Select(menu => menu.BasicId)
            .ToHashSet();
        var retainedMenuIds = existingActiveMenuIds
            .Where(menuId => !autoManagedButtonMenuIds.Contains(menuId))
            .ToHashSet();
        retainedMenuIds.UnionWith(targetButtonMenuIds);
        retainedMenuIds.UnionWith(targetParentMenuIds);

        if (retainedMenuIds.SetEquals(existingActiveMenuIds))
        {
            return;
        }

        await _roleManager.AssignMenusAsync(roleId, retainedMenuIds.ToArray(), tenantId);
    }

    private static bool TryGetButtonPermissionCode(string? permissionCode, out string normalizedPermissionCode)
    {
        normalizedPermissionCode = string.Empty;
        ...
        var operationCode = normalizedCode[(separatorIndex + 1)..].Trim();
        if (!ShouldSyncAsButtonOperation(operationCode))
        {
            return false;
        }

        normalizedPermissionCode = normalizedCode;
        return true;
    }

    private static bool ShouldSyncAsButtonOperation(string operationCode)
    {
        return SupportedButtonOperations.Contains(operationCode);
    }

    private static string BuildScopedMenuCode(long? tenantId, string menuCode)
    {
        return $"{tenantId?.ToString() ?? "0"}:{menuCode.Trim().ToLowerInvariant()}";
    }
}
