...
/// <summary>
/// SaaS 消息模板：登录验证码、找回密码、欢迎信与系统通知的邮件和短信模板（平台全局，租户可建同编码模板覆盖）
/// </summary>
/// <remarks>
/// 代码按模板编码发送，这些模板缺了对应的外发就无法渲染。模板内容归运营：只在首次创建时写入，之后不再覆盖，运营删掉的也不补回。
    ...
    : PlatformDataSeederBase(clientResolver, logger, serviceProvider)
{
    ...
    /// 种子数据优先级
    /// </summary>
    public override int Order => SeedOrders.PlatformData + 2;

    /// <summary>
    /// 种子数据名称
    /// </summary>
    public override string Name => "[SaaS]消息模板";
    ...
    protected override async Task SeedInternalAsync()
    {
        ...
        Logger.LogInformation("{Seeder}：新增 {AddCount} 个，已有的不覆盖", Name, addCount);
    }
