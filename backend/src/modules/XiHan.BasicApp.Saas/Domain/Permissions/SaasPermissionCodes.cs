...
public static class SaasPermissionCodes
{
    ...
    /// </summary>
    /// <remarks>
    /// 由 <see cref="SaasPermissionDefinitions.All"/>（种子落库的单一事实源）派生，不再手工维护清单。
    /// 此前是一份手写的常量枚举，漏登过 <c>saas:log-trace:read</c> —— 手写清单与常量表迟早分叉，
    /// 而"分叉"表现成的是按 All 做白名单/授权枚举时静默少一条权限，没有任何编译期或运行期信号。
    /// </remarks>
    public static IReadOnlyCollection<string> All { get; } = SaasPermissionDefinitions.All
