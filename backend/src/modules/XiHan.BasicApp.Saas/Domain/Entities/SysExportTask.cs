...
// FileName:SysExportTask
// Guid:8d2e5f3b-6c9a-4b4d-ae7f-2a1b3c4d5e6f
// Author:zhaifanhua
...
/// </remarks>
[SugarTable("SysExportTask", "导出任务表")]
...
public partial class SysExportTask : BasicAppFullAuditedEntity
{
    ...
    /// </summary>
    [SugarColumn(ColumnDescription = "业务类型", Length = 100, IsNullable = false)]
    public virtual string BusinessType { get; set; } = string.Empty;

    /// <summary>
    /// 任务名称（用于展示与下载文件名，如「操作日志_20260614120000」）
    /// </summary>
    [SugarColumn(ColumnDescription = "任务名称", Length = 256, IsNullable = false)]
    public virtual string TaskName { get; set; } = string.Empty;

    /// <summary>
    /// 导出范围
    /// </summary>
    [SugarColumn(ColumnDescription = "导出范围", IsNullable = false)]
    public virtual ExportScope Scope { get; set; } = ExportScope.SearchResult;

    /// <summary>
    /// 导出格式
    /// </summary>
    [SugarColumn(ColumnDescription = "导出格式", IsNullable = false)]
    ...
    /// 进度（0-100）
    /// </summary>
    [SugarColumn(ColumnDescription = "进度", IsNullable = false)]
    public virtual int Progress { get; set; }

    /// <summary>
    /// 数据总行数（首页查询得到的 TotalCount）
    /// </summary>
    [SugarColumn(ColumnDescription = "数据总行数", IsNullable = false)]
    public virtual int TotalCount { get; set; }

    /// <summary>
    /// 已处理行数
    /// </summary>
    [SugarColumn(ColumnDescription = "已处理行数", IsNullable = false)]
    public virtual int ProcessedCount { get; set; }

    /// <summary>
    /// 查询条件快照（JSON：资源自身的分页查询 DTO，由 Provider 反序列化为具体类型）
    /// </summary>
    [SugarColumn(ColumnDescription = "查询条件快照(JSON)", ColumnDataType = StaticConfig.CodeFirst_BigString, IsNullable = true)]
    public virtual string? QuerySnapshot { get; set; }

    /// <summary>
    /// 导出列快照（JSON：[{ key, title, valueMap? }]，按顺序写出表头与单元格）
    /// </summary>
    [SugarColumn(ColumnDescription = "导出列快照(JSON)", ColumnDataType = StaticConfig.CodeFirst_BigString, IsNullable = false)]
    public virtual string FieldsSnapshot { get; set; } = string.Empty;

    /// <summary>
    /// 产物文件ID（成功后关联 SysFile.BasicId）
    /// </summary>
    [SugarColumn(ColumnDescription = "产物文件ID", IsNullable = true)]
    public virtual long? FileId { get; set; }

    /// <summary>
    /// 产物文件名（下载展示名）
    /// </summary>
    [SugarColumn(ColumnDescription = "产物文件名", Length = 256, IsNullable = true)]
    public virtual string? FileName { get; set; }

    /// <summary>
    /// 产物文件大小（字节）
    /// </summary>
    [SugarColumn(ColumnDescription = "产物文件大小", IsNullable = false)]
