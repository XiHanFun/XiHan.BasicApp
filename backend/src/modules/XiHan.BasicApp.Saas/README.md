...
全局规则行位于平台库。库隔离（`TenantIsolationMode.Database`）租户的业务数据在租户库，两者不在同一条连接上，此时并入调用方事务只是幻觉——框架不提供两阶段提交，顺序提交中若平台侧失败，调用方会拿到一个既未记录也未推进流水的编号，下一次发号必然重复它。

因此发号器会比较目标库与调用方事务所在库：**跨库时自行开启并提交事务**，编号在返回前即永久生效，调用方事务失败只留下空洞而不会产生重复编号；同库时仍加入调用方事务，保持编号与业务数据同时可见或同时消失。这条判断在 `NumberGenerator` 内完成，调用方无需感知。
