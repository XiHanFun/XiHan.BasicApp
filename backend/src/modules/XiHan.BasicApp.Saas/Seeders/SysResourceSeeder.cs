...
public class SysResourceSeeder : DataSeederBase
{
    ...
    private static List<SysResource> BuildTemplates()
    {
        ...
        [
            ...
            Create("field_level_security", "字段级安全", "/api/field-level-security", "字段级可读可写与脱敏策略管理接口", ResourceType.Api, 170),
            Create("config", "配置中心", "/api/configs", "SaaS 运行配置与租户覆盖接口", ResourceType.Api, 180),
            Create("dict", "字典管理", "/api/dicts", "字典与字典项管理接口", ResourceType.Api, 190),
            Create("notification", "通知公告", "/api/notifications", "通知公告管理接口", ResourceType.Api, 200),
            Create("message", "站内消息", "/api/messages", "站内消息收发与未读管理接口", ResourceType.Api, 210),
            Create("email", "邮件消息", "/api/emails", "邮件投递与审计接口", ResourceType.Api, 220),
            Create("sms", "短信消息", "/api/sms", "短信投递与审计接口", ResourceType.Api, 230),
            Create("oauth_app", "三方应用", "/api/oauth-apps", "OAuth 应用与外部集成接口", ResourceType.Api, 240),
            Create("user_session", "用户会话", "/api/user-sessions", "登录会话与设备策略接口", ResourceType.Api, 250),
            Create("review", "审核流程", "/api/reviews", "审核单据与审批结果接口", ResourceType.Api, 260),
            Create("task", "任务调度", "/api/tasks", "调度任务与触发管理接口", ResourceType.Api, 270),
            Create("file", "文件资源", "/api/files", "文件上传下载与元数据接口", ResourceType.File, 280),
            Create("cache", "缓存管理", "/api/caches", "缓存查询与清理接口", ResourceType.Api, 290),
            Create("monitor", "系统监控", "/api/monitors", "系统监控与节点诊断接口", ResourceType.Api, 300),
            Create("access_log", "访问日志", "/api/access-logs", "访问日志查询接口", ResourceType.Api, 310),
            Create("operation_log", "操作日志", "/api/operation-logs", "操作日志查询接口", ResourceType.Api, 320),
            Create("exception_log", "异常日志", "/api/exception-logs", "异常日志查询接口", ResourceType.Api, 330),
            Create("audit_log", "审计日志", "/api/audit-logs", "审计日志查询接口", ResourceType.Api, 340),
            Create("login_log", "登录日志", "/api/login-logs", "登录日志查询接口", ResourceType.Api, 350),
            Create("task_log", "调度日志", "/api/task-logs", "调度执行日志接口", ResourceType.Api, 360)
        ];
    }
