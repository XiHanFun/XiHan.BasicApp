...
// Guid:10caf66d-9a16-4b6b-b1f4-d7a4832a01ba
// Author:zhaifanhua
...
namespace XiHan.BasicApp.Rbac.Application.Services.Users;

/// <summary>
/// 用户应用服务
/// </summary>
public class UserApplicationService : CrudApplicationServiceBase<SysUser, SysUserDto, long, SysUserCreateDto, SysUserUpdateDto, SysUserPageRequestDto>
{
    ...
    /// 获取租户下的用户列表
    /// </summary>
    public async Task<PageResultDtoBase<SysUserDto>> GetUsersByTenantAsync(long tenantId, PageRequestDtoBase input)
    {
        // 构建租户过滤条件
        var users = await _userRepository.GetUsersByTenantAsync(tenantId);
        var dtos = users.Adapt<List<SysUserDto>>();

        return new PageResultDtoBase<SysUserDto>(dtos, new PageResultMetadata
        {
            PageIndex = 1,
            PageSize = dtos.Count,
