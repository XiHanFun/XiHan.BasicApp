...
// Guid:29f7d4c1-63a8-4e50-b9d2-05c81e37f6a4
// Author:zhaifanhua
// Email:me@zhaifanhua.com
// CreateTime:2026/07/17 10:32:00
// ----------------------------------------------------------------
...
/// </summary>
/// <remarks>
/// 播种工作流模块所需的通用操作字典（read/create/update/delete/execute），作为权限点的"动作"维度。
/// 必须先于 <see cref="SysPermissionSeeder"/> 执行：后者按「资源 × 操作」派生 workflow:* 权限，
/// 若本表为空则整条工作流权限/菜单/授权链在干净库上静默跳过。
/// 操作字典为全局共享（TenantId=0），与其它模块的同名操作幂等共存（按编码去重）。
/// </remarks>
public class SysOperationSeeder : DataSeederBase
{
    ...
    /// <summary>
    /// 种子数据优先级（工作流种子统一在 Order 300+ 独立段，与 Saas 10-37/代码生成 100-105/AI 200-212 不交叠）
    /// </summary>
    ...
    /// 种子数据名称
    /// </summary>
    public override string Name => "[Workflow]系统操作种子数据";
