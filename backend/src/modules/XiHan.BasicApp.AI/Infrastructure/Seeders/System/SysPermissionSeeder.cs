...
public class SysPermissionSeeder : PlatformDataSeederBase
{
    ...
    /// <summary>
    /// 种子数据优先级（须先于 AiMenuSeeder，菜单建立时即可解析 ai:read 绑定可见性）
    /// </summary>
    ...
    {
        var client = DbClient;
        var resources = await client.Queryable<SysResource>().Where(r => r.ResourceCode == AiPermissionCodes.Resource).ToListAsync();
        ...
        {
            [AiPermissionCodes.Resource] = ["read", "create", "update", "delete", "execute"]
        };
