...
// FileName:PromptRolePermissionSeeder
// Guid:a11c0de0-a004-4a10-9a00-00000000aia3
// Author:zhaifanhua
...
/// AI 提示词库角色权限种子数据
/// </summary>
/// <remarks>提示词库属平台级开发工具：仅授予超级管理员角色。菜单可见性绑定已前移至菜单种子。</remarks>
public class PromptRolePermissionSeeder : DataSeederBase
{
    /// <summary>
    /// 构造函数
    /// </summary>
    public PromptRolePermissionSeeder(ISqlSugarClientResolver clientResolver, ILogger<PromptRolePermissionSeeder> logger, IServiceProvider serviceProvider)
    ...
    /// 种子数据优先级
    /// </summary>
    public override int Order => 212;

    /// <summary>
    /// 种子数据名称
    /// </summary>
    public override string Name => "[Ai]提示词库角色权限种子数据";
    ...
    protected override async Task SeedInternalAsync()
    {
        ...
        var permissions = await client.Queryable<SysPermission>()
            .Where(p => p.PermissionCode.StartsWith("ai_prompt:"))
            .ToListAsync();
        if (permissions.Count == 0)
        {
            Logger.LogWarning("AI 提示词库权限不存在，跳过提示词库角色权限种子"); return;
        }
        ...
        Logger.LogInformation("AI 提示词库仅授超级管理员：新增角色权限 {GrantCount} 条", grantedCount);
    }
}
