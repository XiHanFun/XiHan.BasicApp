...
public class SysResourceSeeder : PlatformDataSeederBase
{
    ...
    {
        var client = DbClient;
        var exists = await client.Queryable<SysResource>().Where(r => r.ResourceCode == AiPermissionCodes.Resource).ToListAsync();
        var existsCodes = exists.Select(x => x.ResourceCode).ToHashSet();
        var addList = new List<SysResource>();

        if (!existsCodes.Contains("ai"))
        {
            addList.Add(new SysResource { ResourceCode = AiPermissionCodes.Resource, ResourceName = "AI 服务", ResourceType = ResourceType.Api, ResourcePath = "/api/ai", Description = "AI 服务API接口", AccessLevel = ResourceAccessLevel.Authorized, Status = EnableStatus.Enabled, Sort = 402 });
        }
