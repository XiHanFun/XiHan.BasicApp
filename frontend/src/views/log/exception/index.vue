...
const fields = computed<ListFieldSchema[]>(() => [
  ...
  { key: 'exceptionLocation', title: t('log.exception.exception_location'), dataType: 'string', advancedSearch: true, minWidth: 200, order: 18 },
  {
    ...
    width: 90,
    order: 19,
    render: row => h(XhTagRoot, { variant: 'solid', tone: severityType((row as unknown as ExceptionLogListItemDto).severityLevel) }, () => h(XhTagLabel, () => getOptionLabel(severityOptions.value, (row as unknown as ExceptionLogListItemDto).severityLevel))),
  },
  ...
  { key: 'exceptionTime', title: t('log.exception.exception_time'), dataType: 'datetime', sortable: true, minWidth: 170, order: 36 },
  {
    ...
    width: 100,
    order: 37,
    render: row => h(XhTagRoot, { variant: 'solid', tone: (row as unknown as ExceptionLogListItemDto).isHandled ? 'success' : 'warning' }, () => h(XhTagLabel, () => (row as unknown as ExceptionLogListItemDto).isHandled ? t('log.exception.handled') : t('log.exception.unhandled'))),
  },
