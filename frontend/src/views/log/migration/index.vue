...
const fields = computed<ListFieldSchema[]>(() => [
  ...
  { key: 'scriptName', title: t('log.migration.script_name'), dataType: 'string', searchable: true, sortable: true, searchPlaceholder: t('log.migration.script_name_placeholder'), minWidth: 200, order: 11 },
  {
    ...
    width: 110,
    order: 12,
    render: row => h(XhTagRoot, { variant: 'outline', tone: (row as unknown as MigrationHistoryListItemDto).success ? 'success' : 'danger' }, () => h(XhTagLabel, () => (row as unknown as MigrationHistoryListItemDto).success ? t('log.migration.success_yes') : t('log.migration.success_no'))),
  },
