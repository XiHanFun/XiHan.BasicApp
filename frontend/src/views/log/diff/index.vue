...
const fields = computed<ListFieldSchema[]>(() => [
  ...
  { key: 'changeDescription', title: t('log.diff.change_description'), dataType: 'string', minWidth: 220, order: 18 },
  {
    ...
    render: (row) => {
      ...
      return h(XhTagRoot, { variant: 'solid', tone: riskTagType(level) }, () => h(XhTagLabel, () => getOptionLabel(riskLevelOptions.value, level)))
    },
  },
  {
    key: 'isSuccess',
    ...
    width: 90,
    order: 20,
    render: row => h(XhTagRoot, { variant: 'solid', tone: (row as unknown as DiffLogListItemDto).isSuccess ? 'success' : 'danger' }, () => h(XhTagLabel, () => (row as unknown as DiffLogListItemDto).isSuccess ? t('log.diff.result_success') : t('log.diff.result_failed'))),
  },
