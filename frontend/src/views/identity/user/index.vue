...
const fields = computed<ListFieldSchema[]>(() => [
  ...
  // 性别（常用搜索 + 列）
  {
    ...
    render: (row) => {
      ...
      return h(XhTagRoot, { variant: 'solid', tone: GENDER_TAG_TYPE[r.gender] ?? 'neutral', style: { fontWeight: 500 } }, () => h(XhTagLabel, () => label))
    },
  },
  // 地区/语言（仅列）
  {
  ...
  // 账号状态（常用搜索 + 列）
  {
    ...
    render: (row) => {
      const r = row as unknown as UserListItemDto
      return h(XhTagRoot, { variant: 'solid', tone: r.status === EnableStatus.Enabled ? 'success' : 'danger', style: { fontWeight: 500 } }, () => h(XhTagLabel, () => (r.status === EnableStatus.Enabled ? t('identity.user.status_enabled') : t('identity.user.status_disabled'))))
    },
  },
