...
defineOptions({ name: 'WorkbenchWidgetsPage' })
...
// 同步开关在「偏好设置 → 同步 → 小组件看板」，本页只读 appStore.widgetsSyncEnabled 显示状态徽标
...
function persist() {
  ...
  // 本地始终留一份（关闭同步时即以此为准）；sync.save 内部按「小组件看板同步」开关决定是否上行后端
  try {
        ...
        >
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <span class="text-sm font-semibold text-foreground">{{ t('menu.workbench_widgets') }}</span>
