...
import { NButton, NModal, NPopconfirm, NSelect, NSpace, NTag, useMessage } from 'naive-ui'
      ...
      },
      {
        ...
        formatter: ({ cellValue }) =>
          cellValue !== undefined && cellValue !== null
            ? getOptionLabel(REVIEW_RESULT_OPTIONS, cellValue)
            : '-',
      },
...
function getReviewStatusType(status: number) {
  const map: Record<number, 'default' | 'info' | 'success' | 'error' | 'warning'> = {
    0: 'default',
    1: 'info',
    2: 'success',
    3: 'error',
  ...
  <div class="flex overflow-hidden flex-col gap-2 p-3 h-full">
    ...
    >
      <div class="space-y-3 text-sm">
        <div>
          <span class="font-medium text-gray-500">审查编码：</span>
          {{ detailData.reviewCode }}
        </div>
        <div>
          <span class="font-medium text-gray-500">审查标题：</span>
          {{ detailData.reviewTitle }}
        </div>
        <div>
          <span class="font-medium text-gray-500">审查类型：</span>
          {{ detailData.reviewType }}
        </div>
        <div>
          <span class="font-medium text-gray-500">提交时间：</span>
