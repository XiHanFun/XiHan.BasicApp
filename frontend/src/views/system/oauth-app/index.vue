...
async function handleSubmit() {
  try {
    ...
    if (formData.value.basicId) await oauthAppApi.update(formData.value.basicId, formData.value)
    ...
    <vxe-card class="flex-1" style="height: 0">
      <vxe-grid ref="xGrid" v-bind="options">
        <template #toolbar_buttons>
          <NButton type="primary" size="small" @click="handleAdd">新增应用</NButton>
        </template>
    ...
    >
      <NForm :model="formData" label-placement="left" label-width="120px">
        ...
        <NFormItem label="授权类型" path="grantTypes">
          <NInput
            v-model:value="formData.grantTypes"
            placeholder="如: authorization_code,refresh_token"
          />
        </NFormItem>
