...
function handleQueryApi(page: VxeGridPropTypes.ProxyAjaxQueryPageParams) {
  return requestClient.post('/api/ExceptionLog/Page', buildPageRequest({
...
const options = useVxeTable({
  ...
  columns: [
    ...
    { field: 'handledTime', title: '处理时间', width: 170, formatter: ({ cellValue }) => formatDate(cellValue) },
    { field: 'exceptionTime', title: '异常时间', width: 170, formatter: ({ cellValue }) => formatDate(cellValue), sortable: true },
  ...
  <div class="h-full flex flex-col gap-2 overflow-hidden p-3">
    ...
    >
      <div class="space-y-3 text-sm">
        <div><span class="font-medium text-gray-500">异常类型：</span>{{ detailData.exceptionType }}</div>
        <div><span class="font-medium text-gray-500">异常消息：</span>{{ detailData.exceptionMessage }}</div>
        <div><span class="font-medium text-gray-500">异常来源：</span>{{ detailData.exceptionSource }}</div>
        <div><span class="font-medium text-gray-500">异常位置：</span>{{ detailData.exceptionLocation }}</div>
        <div><span class="font-medium text-gray-500">请求路径：</span>{{ detailData.requestMethod }} {{ detailData.requestPath }}</div>
        <div><span class="font-medium text-gray-500">控制器/操作：</span>{{ detailData.controllerName }}.{{ detailData.actionName }}</div>
        <div><span class="font-medium text-gray-500">错误码：</span>{{ detailData.errorCode }}</div>
        <div><span class="font-medium text-gray-500">业务模块：</span>{{ detailData.businessModule }}</div>
        <div><span class="font-medium text-gray-500">异常时间：</span>{{ formatDate(detailData.exceptionTime) }}</div>
        <div v-if="detailData.handledByName">
          <span class="font-medium text-gray-500">处理人：</span>{{ detailData.handledByName }}
          <span class="ml-4 font-medium text-gray-500">处理时间：</span>{{ formatDate(detailData.handledTime) }}
        </div>
        <div v-if="detailData.handledRemark">
          <span class="font-medium text-gray-500">处理备注：</span>{{ detailData.handledRemark }}
        </div>
        <div v-if="detailData.exceptionStackTrace">
          <span class="font-medium text-gray-500">堆栈信息：</span>
          <pre class="mt-1 max-h-60 overflow-auto rounded bg-gray-100 p-3 text-xs dark:bg-gray-800">{{ detailData.exceptionStackTrace }}</pre>
        </div>
        <div v-if="detailData.innerExceptionType">
          <span class="font-medium text-gray-500">内部异常：</span>{{ detailData.innerExceptionType }} - {{ detailData.innerExceptionMessage }}
        </div>
        <div v-if="detailData.innerExceptionStackTrace">
          <span class="font-medium text-gray-500">内部堆栈：</span>
          <pre class="mt-1 max-h-40 overflow-auto rounded bg-gray-100 p-3 text-xs dark:bg-gray-800">{{ detailData.innerExceptionStackTrace }}</pre>
        </div>
      </div>
    </NModal>
  </div>
