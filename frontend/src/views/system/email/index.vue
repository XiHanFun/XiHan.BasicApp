...
function resetForm() {
  formData.value = { fromEmail: '', toEmail: '', subject: '', content: '', emailType: 0, isHtml: false }
}
function handleAdd() {
  ...
  <div class="h-full flex flex-col">
    <vxe-card class="mb-2" style="padding: 10px 16px">
      <div class="flex items-center gap-3 flex-wrap">
        <vxe-input v-model="queryParams.keyword" placeholder="搜索发件人/收件人/主题" clearable style="width: 280px" @keyup.enter="handleSearch" />
        <NSelect v-model:value="queryParams.emailType" :options="EMAIL_TYPE_OPTIONS" placeholder="邮件类型" clearable style="width: 140px" />
        <NSelect v-model:value="queryParams.emailStatus" :options="EMAIL_STATUS_OPTIONS" placeholder="发送状态" clearable style="width: 130px" />
    ...
    <NModal v-model:show="modalVisible" :title="modalTitle" preset="card" style="width: 600px" :auto-focus="false">
      <NForm :model="formData" label-placement="left" label-width="80px">
        ...
        <NFormItem label="内容" path="content">
          <NInput v-model:value="formData.content" type="textarea" :rows="5" placeholder="邮件内容" />
        </NFormItem>
      </NForm>
      <template #footer>
        <NSpace justify="end">
