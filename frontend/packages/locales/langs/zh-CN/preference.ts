export default {
  sync_status: {
    local: '本地',
    local_tip: '仅本地保存，未开启同步（可在 偏好设置 › 通用 中开启对应同步）',
    synced: '同步',
    synced_tip: '已开启同步，变更会保存到后端账号并跨端覆盖',
  },
  drawer: {
    title: '偏好设置',
    save: '保存',
    copy: '复制偏好设置',
    reset: '重置偏好',
    clear_cache: '清除缓存并登出',
    save_success: '偏好设置已保存',
    copy_success: '偏好设置已复制',
    copy_failed: '复制失败',
    reset_success: '已恢复默认，点击保存后生效',
    tab: {
      appearance: '外观',
      layout: '布局',
      shortcut: '快捷键',
      general: '通用',
    },
  },
  appearance: {
    mode: {
      title: '模式',
      light: '浅色',
      dark: '深色',
      auto: '跟随系统',
    },
    color: {
      title: '颜色',
      default_or_custom: '默认或自定义',
      default: '默认',
      custom: '自定义',
      dynamic: 'Material You 动态取色',
      dynamic_tip: '从品牌色自动推导整套和谐色阶（辅色 / 容器 / 前景 / 边框等），关闭则仅改主色',
      family: {
        red: '红',
        orange: '橙',
        yellow: '黄',
        green: '绿',
        cyan: '青',
        blue: '蓝',
        purple: '紫',
      },
      preset: {
        yan_zhi_hong: '胭脂红',
        zhu_sha_hong: '朱砂红',
        jiang_zi_hong: '绛紫红',
        zhe_shi_zong: '赭石棕',
        zhuan_wa_cheng: '砖瓦橙',
        hu_po_cheng: '琥珀橙',
        jiang_huang_cheng: '姜黄橙',
        xiang_ye_huang: '缃叶黄',
        teng_huang_se: '藤黄色',
        song_hua_lv: '松花绿',
        zhu_qing_lv: '竹青绿',
        bi_yu_lv: '碧玉绿',
        bi_bo_qing: '碧波青',
        shi_qing_se: '石青色',
        fei_cui_qing: '翡翠青',
        cang_qing_lan: '藏青蓝',
        shi_ban_lan: '石板蓝',
        ji_lan_se: '霁蓝色',
        ding_xiang_zi: '丁香紫',
        ou_he_zi: '藕荷紫',
        qing_lian_zi: '青莲紫',
      },
    },
    radius: {
      title: '圆角',
    },
    navigation: {
      title: '导航',
      sidebar_dark: '深色侧边栏',
      sidebar_sub_dark: '深色侧边栏子栏',
      header_dark: '深色顶栏',
      sidebar_dark_tip: '在浅色主题下，单独将侧边栏切换为深色风格',
      sidebar_sub_dark_tip: '当主题为浅色，开启深色侧边栏且布局使用双列菜单模式时可开启',
      header_dark_tip: '在浅色主题下，单独将顶栏切换为深色风格',
    },
    font: {
      title: '字体',
      size: '大小',
    },
    interaction: {
      title: '交互',
    },
    effects: {
      title: '效果',
    },
    other: {
      title: '其它',
      color_weakness: '色弱模式',
      grayscale: '灰色模式',
      color_weakness_tip: '色觉辅助模式，适合红绿色觉异常用户，通过滤镜提升颜色辨识度',
      grayscale_tip: '将整个页面渲染为灰度，常用于特殊纪念日',
    },
  },
  layout: {
    title: '布局',
    preset: {
      side: '垂直',
      side_mixed: '双列',
      top: '水平',
      mix: '混合垂直',
      header_mix: '混合双列',
      header_sidebar: '侧边',
      full: '内容全屏',
    },
    preset_tip: {
      side: '左侧固定侧边栏 + 右侧内容区，适合层级较深的多级菜单',
      side_mixed: '左侧一级图标栏 + 二级文字菜单，节省横向空间',
      top: '顶部水平导航，适合菜单项较少的扁平结构',
      mix: '顶部一级菜单 + 左侧二级菜单，适合大型多模块应用',
      header_mix: '顶部一级 + 双列侧边二级，兼顾导航宽度与层级',
      header_sidebar: '顶部品牌栏 + 左侧完整菜单，经典后台风格',
      full: '隐藏所有导航，内容占满全屏，适合专注操作场景',
    },
    content: {
      title: '内容',
      fluid: '流式',
      fixed: '定宽',
      fluid_tip: '内容区随窗口宽度自由伸缩，充分利用屏幕空间',
      fixed_tip: '内容区设置最大宽度并居中，适合宽屏阅读场景',
    },
    sidebar: {
      title: '侧边栏',
      show: '显示',
      collapse: '折叠菜单',
      hover_expand: '鼠标悬停展开',
      collapsed_show_title: '折叠显示菜单名',
      auto_activate_child: '自动激活子菜单',
      show_buttons: '显示按钮',
      collapse_button: '折叠按钮',
      fixed_button: '固定按钮',
      width: '宽度',
      hover_expand_tip: '需要先开启「折叠菜单」，鼠标悬停侧边栏时临时展开，移走后自动收起',
      collapsed_show_title_tip: '侧边栏折叠后，在菜单图标下方显示对应的文字名称',
      auto_activate_child_tip: '悬停展开折叠侧边栏时，自动高亮第一个可用子菜单项',
    },
    header: {
      title: '顶栏',
      show: '显示',
      mode: '模式',
      mode_tip: '固定：顶栏始终吸顶，滚动内容时保持可见；静态：顶栏随页面一起滚动、不吸顶。',
      mode_fixed: '固定',
      mode_static: '静态',
      menu_align: '菜单位置',
      menu_align_left: '左侧',
      menu_align_center: '居中',
      menu_align_right: '右侧',
    },
    navigation: {
      title: '导航菜单',
      style: '风格',
      style_rounded: '圆润',
      style_plain: '朴素',
      split: '分离',
      accordion: '手风琴模式',
      split_tip: '将侧边导航菜单与内容区域分离，以独立面板形式展示',
      accordion_tip: '展开一个菜单项时，自动折叠同级的其他菜单项',
    },
    breadcrumb: {
      title: '面包屑导航',
      enabled: '启用',
      hide_only_one: '仅有一个时隐藏',
      show_icon: '显示图标',
      show_home: '显示首页按钮',
      style: '风格',
      style_normal: '常规',
      style_background: '背景',
      hide_only_one_tip: '当前路径层级只有一项时，自动隐藏整个面包屑',
      nav_buttons: '显示前进/后退按钮',
      nav_buttons_tip: '在工具栏中显示浏览历史的前进、后退导航按钮',
    },
    tabbar: {
      title: '标签栏',
      enabled: '启用',
      persist: '持久化',
      visit_history: '访问历史记录',
      max_count: '最大数量',
      max_count_unit: '个',
      draggable: '拖拽排序',
      scroll_response: '纵向滚轮响应',
      middle_click_close: '鼠标中键关闭',
      show_icon: '显示图标',
      show_more: '显示更多按钮',
      show_overview: '显示标签总览按钮',
      show_maximize: '显示最大化按钮',
      style: '风格',
      style_chrome: '谷歌',
      style_plain: '简约',
      style_card: '卡片',
      style_brisk: '活泼',
      persist_tip: '关闭浏览器后再次打开，自动恢复上次的标签页状态',
      visit_history_tip: '记录标签访问历史，支持通过前进/后退操作在已访问标签间切换',
      max_count_tip: '超出数量限制时，最早打开的标签将被自动关闭，设为 0 表示不限制',
      draggable_tip: '按住标签并拖动可调整标签的排列顺序',
      scroll_response_tip:
        '开启后标签栏可响应鼠标纵向滚轮事件。\n关闭时只能响应系统横向滚动（需按住 Shift 再滚动）',
      middle_click_close_tip: '使用鼠标中键点击标签页可快速将其关闭',
    },
    widget: {
      title: '工具栏',
      global_search: '显示命令面板按钮',
      theme_toggle: '显示主题切换按钮',
      language_toggle: '显示语言切换按钮',
      timezone_toggle: '显示时区切换按钮',
      fullscreen: '显示全屏按钮',
      notification: '显示通知按钮',
      lock_screen: '显示锁屏按钮',
      sidebar_toggle: '显示侧栏切换按钮',
      refresh: '显示刷新按钮',
      favorites: '显示收藏夹按钮',
      preference_position: '偏好设置位置',
      preference_position_auto: '自动',
      preference_position_header: '顶栏',
      preference_position_floating: '悬浮',
    },
    footer: {
      title: '底栏',
      show: '显示',
      fixed: '固定在底部',
      show_dev_info: '显示开发版本信息',
    },
    copyright: {
      title: '版权',
      enabled: '启用',
      name: '名称',
      site: '主页',
      start_date: '起始年份',
      icp: 'ICP 备案号',
      icp_url: 'ICP 网站链接',
      optional: '选填',
    },
  },
  general: {
    title: '通用',
    sync_title: '同步',
    update_title: '更新',
    language: '语言',
    timezone: '时区',
    dynamic_title: '动态标题',
    dynamic_island: '灵动岛',
    notify_sound: '消息提示音',
    table_row_peek: '速览卡',
    preference_sync: '偏好设置',
    favorites_sync: '收藏夹设置',
    search_sync: '搜索设置',
    table_sync: '表格设置',
    widgets_sync: '仪表盘看板',
    watermark: '水印',
    watermark_content: '内容',
    watermark_text: '水印文案',
    check_updates: '定时检查',
    check_updates_interval: '检查间隔',
    check_updates_interval_unit: '秒',
    dynamic_title_tip: '页面标题随当前路由名称动态更新，便于在多个浏览器标签页中区分',
    dynamic_island_tip: '顶部灵动岛：登录、设置同步、文件上传、网络状态等过程会在此聚合提示，点击可展开查看活动与历史；关闭后这些结果改用消息提示框显示。',
    notify_sound_tip: '收到他人发来的聊天消息或站内通知时播放提示音，两者音色不同；每条都会响，密集到达时依次岔开播放；浏览器要求页面上发生过点击等操作后才允许发声。',
    table_row_peek_tip: '在各列表页悬停某一行约半秒后，于光标旁浮出该行全部字段的速览卡（含表格隐藏列）；关闭后不再弹出。移动端不生效。',
    preference_sync_tip: '默认开启，偏好变更同步到后端账号，并实时推送到该账号的其它在线设备；关闭后仅保存在本机。',
    favorites_sync_tip: '默认开启，收藏夹变更同步到后端账号，并实时推送到其它在线设备；关闭后仅保存在本机。',
    search_sync_tip: '默认开启，各列表页的搜索条件设置保存时同步到后端账号，并实时推送到其它在线设备；关闭后仅保存在本机。',
    table_sync_tip: '默认开启，各列表页的表格列设置与个人视图保存时同步到后端账号，并实时推送到其它在线设备；关闭后仅保存在本机。',
    widgets_sync_tip: '默认开启，仪表盘看板的布局（顺序与宽度）保存时同步到后端账号，并实时推送到其它在线设备；关闭后仅保存在本机。',
    watermark_tip: '在页面正文区域叠加半透明水印文字',
    check_updates_tip: '定期检测是否有新版本可用，有更新时显示提示',
    frosted_glass: '磨砂效果',
    frosted_glass_tip: '在整个页面叠加类似磨砂质感的噪点纹理，效果与偏好设置中的色块一致',
    frosted_glass_intensity: '纹理强度',
    animation: {
      title: '动画',
      transition_progress: '页面切换进度条',
      transition_loading: '页面加载动画',
      theme_animation: '主题切换',
      transition_enable: '页面切换',
      fade: '淡入淡出',
      slide_left: '左右滑动',
      slide_right: '反向滑动',
      slide_up: '向上滑入',
      slide_down: '向下滑入',
      zoom_fade: '缩放淡入',
      flip_fade: '翻转淡入',
      scale_up: '放大进入',
      scale_down: '缩小进入',
      blur_fade: '模糊淡入',
      rotate_fade: '旋转淡入',
      skew_slide: '倾斜滑入',
      transition_progress_tip: '页面跳转时在顶部显示细进度条，指示加载进度',
      transition_loading_tip: '页面跳转时在内容区域显示加载动画遮罩',
      theme_animation_tip: '主题切换时从鼠标点击位置扩散的圆形波纹动画，\n仅在切换至暗色主题时触发',
      transition_enable_tip: '开启后页面间以动画平滑过渡，关闭后瞬间切换',
      loading_fixed_color: '固定颜色',
      loading_fixed_color_tip: '开启后加载动画使用固定前景色（暗色主题为亮色、亮色主题为暗色），不跟随主题色',
      loaders: {
        'original-thinking': '原始思考',
        'thinking-five': '五瓣思考',
        'thinking-nine': '九瓣思考',
        'rose-orbit': '玫瑰轨道',
        'rose-curve': '玫瑰曲线',
        'rose-two': '玫瑰·二瓣',
        'rose-three': '玫瑰·三瓣',
        'rose-four': '玫瑰·四瓣',
        'lissajous-drift': '利萨茹漂移',
        'lemniscate-bloom': '双纽线绽放',
        'hypotrochoid-loop': '内旋轮线',
        'three-petal-spiral': '三瓣螺旋',
        'four-petal-spiral': '四瓣螺旋',
        'five-petal-spiral': '五瓣螺旋',
        'six-petal-spiral': '六瓣螺旋',
        'butterfly-phase': '蝴蝶曲线',
        'cardioid-glow': '心形辉光',
        'cardioid-heart': '心形线',
        'heart-wave': '心形波',
        'spiral-search': '螺旋搜索',
        'fourier-flow': '傅里叶流',
      },
    },
  },
  shortcut: {
    global: '全局',
    enabled: '快捷键',
    search: '命令面板',
    logout: '退出登录',
    lock: '锁定屏幕',
    tab_overview: '标签总览',
    enabled_tip: '开启后，下方各快捷键将在全局范围内持续生效',
    search_tip: '在任意页面按下快捷键，打开命令面板（搜索菜单与操作，支持拼音）',
    lock_tip: '按下快捷键后立即锁定屏幕，需要输入密码才能恢复使用',
    logout_tip: '按下快捷键后直接退出登录并跳转至登录页',
    tab_overview_tip: '按下快捷键打开标签卡片总览（已打开标签的网格视图），再次按下关闭',
  },
}
