来源：https://basicapp.docs.xihanfun.com/frontend/i18n
...
| 步骤 | 做什么 |
| --- | --- |
| 1 | `packages/locales/langs/` 与 `src/locales/langs/` 下各加一套语言目录，按模块补齐文案；插件模块在 `src/modules/<模块>/locales/<locale>.ts` 各补一份 |
| 2 | 在 `packages/constants/app.ts` 的 `SUPPORTED_LOCALES`、`packages/locales/index.ts` 的 `messages` 与 `src/locales/index.ts` 里登记新语言，并在 `LocaleSwitcher.vue` 的 `LOCALES`、`LocaleFlag.vue` 的国旗、`packages/locales/xihan-ui.ts` 的 `xhTranslations` 各加一项 |
| 3 | **后端**补一份 `Enums.{culture}.json` 与业务资源文件（`Localization/ApiResponse|Enums|Errors.{culture}.json`） |
| 4 | **后端每个环境**的 `XiHan:Localization:SupportedCultures` 加上该文化——文化中间件只接受列表内的 `X-Language`，不在列内一律回退默认文化；`appsettings.Production.json` 不入库，要在服务器上单独改 |

第 3、4 步最容易漏——前端切了语言，接口返回的 `message` 和枚举标签却没变，要么是后端资源没补，要么是生产环境的 `SupportedCultures` 没登记（de-DE 上线时就漏过一次）。
